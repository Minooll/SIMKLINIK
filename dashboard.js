(() => {
  ...
  document.getElementById('statsGrid').innerHTML = data.stats.map(([label, value, note, icon, color]) => `<article class="stat-card"><div class="stat-top"><span>${label}</span><span class="stat-icon" style="background:${color === 'mint' ? '#e4f4f0' : color === 'blue' ? '#e4f0f7' : color === 'gold' ? '#fff1d9' : '#fce9e4'}">${icon}</span></div><strong class="stat-value">${value}</strong><small class="stat-note ${note.startsWith('+') ? 'up' : ''}">${note}</small></article>`).join('');
